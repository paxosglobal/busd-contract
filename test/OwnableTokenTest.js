const BUSD = artifacts.require('BUSDImplementation.sol');
...
contract('Ownable BUSD', function ([_, admin, anotherAccount, anotherAccount2, owner]) {
  beforeEach(async function () {
    const busd = await BUSD.new({from: owner});
  ...
  describe('as an ownable', function () {
    ...
    it('sets new potential owner after proposeOwner', async function () {
      ...
      // emits an OwnershipTransferredStarted event
    ...
    it('allows owner to removes proposed owner after disregardProposeOwner', async function () {
      await this.token.proposeOwner(anotherAccount, {from: owner});
      const {logs} = await this.token.disregardProposeOwner({from: owner});
      let currentOwner = await this.token.owner();
      assert.strictEqual(currentOwner, owner);

      // emits an OwnershipTransferredDisregarded event
      assert.strictEqual(logs.length, 1);
    ...
    it('allows proposed to removes proposed owner after disregardProposeOwner', async function () {
      await this.token.proposeOwner(anotherAccount, {from: owner});
  ...
  describe('as an initializable token', function () {
    ...
    it('constructor initializes the implementation contract and pauses it to avoid misleading state there', async function () {
      const isPaused = await this.busd.paused();
      assert.strictEqual(isPaused, true);
      const currentOwner = this.busd.owner();
      assert.notStrictEqual(currentOwner, ZERO_ADDRESS);
      await assertRevert(this.busd.initialize());
    });
  });
});
